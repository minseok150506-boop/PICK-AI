...
from question_rewriter import rewrite_for_reasoning, build_clarification
...
def orchestrate(user_message: str, history: list[dict] | None = None) -> OrchestrationResult:
    history = history or []

    analysis = analyze_question(user_message, history)
    context = resolve_reference(user_message, history)
    route = plan_route(analysis, context)
