...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "pick-search-time-secret")
...
SEARCH_WORDS = ["검색", "찾아", "찾아줘", "최신", "뉴스", "사이트", "링크", "유튜브", "youtube", "영상", "쇼츠", "가격", "날씨"]
TIME_WORDS = ["몇 시", "몇시", "시간", "오늘 날짜", "날짜", "지금"]
...
def is_time_question(message):
    return has_any(message, TIME_WORDS)

def needs_search(message):
    return has_any(message, SEARCH_WORDS)

def youtube_link(query):
    q = query.replace("유튜브", "").replace("youtube", "").replace("영상", "").strip() or query
...
def duckduckgo_search(query):
    try:
        ...
        out = []
        for link, title in pattern.findall(res.text)[:5]:
...
def search_context(message):
    parts = []
    if "유튜브" in message.lower() or "youtube" in message.lower() or "영상" in message:
        parts.append("유튜브 검색 링크: " + youtube_link(message))
    if needs_search(message):
        result = duckduckgo_search(message)
        if result:
...
def ask_ollama(user_message, chat_id=None, user_id=None):
    ...
    if is_time_question(user_message) and not needs_search(user_message):
    ...
    if chat_id and user_id:
        ...
        if history:
            prompt += "\n\n이전 대화:\n" + history
    ctx = search_context(user_message)
    if ctx:
        prompt += "\n\n인터넷/유튜브 참고 정보:\n" + ctx
    prompt += f"\n\n사용자: {user_message}\nPICK:"
...
@app.route("/healthz")
def healthz():
    return jsonify({"ok": True, "service": "PICK Ollama Search Time", "model": OLLAMA_MODEL, "time": now_text()})
